...
library(reshape2)
#based on median
top_int_data.median<-perobindall.cor[perobindall.cor$Image_Metadata_array<9,"Cell_Intensity_MedianIntensity_Icam1amask_Nor"]
pos_int_data<-perobindall.cor[perobindall.cor$Image_Metadata_array==9,"Cell_Intensity_MedianIntensity_Icam1amask_Nor"]
neg_int_data<-perobindall.cor[perobindall.cor$Image_Metadata_array==10,"Cell_Intensity_MedianIntensity_Icam1amask_Nor"]
lower.limit <- 0.02
upper.limit <- 0.2
...
intersection.point.median<-min(intersection.point)

combined<-perobindall.cor[perobindall.cor$Image_Metadata_array>8,c("Image_Metadata_array",
                                                                   "Cell_Intensity_MedianIntensity_Icam1amask_Nor")]
ggplot(combined, aes(Cell_Intensity_MedianIntensity_Icam1amask_Nor, fill =as.factor(Image_Metadata_array))) +
  geom_density(alpha = 0.2) + 
  geom_vline(xintercept = intersection.point.median, color = "red")
##estimate percentage of positive cells in negative control
percentile <- ecdf(neg_int_data)
ad.treshhold<-percentile(intersection.point.median)

#########################################################
###adopting treshhold to each repeat######################
###############################################################
#################################################################
##ICAM analysis

top_int_data.median<-perobindall.cor[perobindall.cor$Image_Metadata_array<9,
                                     c("Cell_Intensity_MedianIntensity_Icam1amask_Nor","ImageNumber","FeatureIdx","Image_Metadata_array")]
library(plyr)
ad.treshhold.t<-ddply(top_int_data.median,"Image_Metadata_array", summarise, 
                     Treshold=quantile(Cell_Intensity_MedianIntensity_Icam1amask_Nor,ad.treshhold))
top_int_data.median.t<-merge(ad.treshhold.t,top_int_data.median,by="Image_Metadata_array")

neg_int_data.median<-perobindall.cor[perobindall.cor$Image_Metadata_array==10,
                                     c("Cell_Intensity_MedianIntensity_Icam1amask_Nor","ImageNumber","FeatureIdx")]


#count number of postitive cells, with ICAm expression higher then treshhold
library(plyr)
ratiorankcorr<-ddply(top_int_data.median.t,"ImageNumber", summarise, 
                     IcamPositive=sum(Cell_Intensity_MedianIntensity_Icam1amask_Nor > Treshold),
                     Total=sum(Cell_Intensity_MedianIntensity_Icam1amask_Nor > 0),
                     TrMeanIcam=mean(Cell_Intensity_MedianIntensity_Icam1amask_Nor, trimm=0.2),
                     FeatureIdx=unique(FeatureIdx))

ratiorankcorrn<-ddply(neg_int_data.median,"ImageNumber", summarise, 
                      IcamPositive=sum(Cell_Intensity_MedianIntensity_Icam1amask_Nor >intersection.point.median),
                      Total=sum(Cell_Intensity_MedianIntensity_Icam1amask_Nor > 0),
                      TrMeanIcam=mean(Cell_Intensity_MedianIntensity_Icam1amask_Nor, trimm=0.2),
                      FeatureIdx=unique(FeatureIdx))
...
##show boxplot with treshhold
##before correction
ggplot(perobindall.cor,aes(factor(Image_Metadata_array), 
...
ggplot(perobindall.cor,aes(factor(Image_Metadata_array), 
                    ...
                    fill=factor(Image_Metadata_array)))+geom_boxplot()
#after noise subtraction plus adaptive treshhold
ggplot(perobindall.cor[perobindall.cor$Image_Metadata_array<9,],aes(factor(Image_Metadata_array), 
          Cell_Intensity_MeanIntensity_Icam1amask_Nor,
                      fill=factor(Image_Metadata_array)))+geom_boxplot()+
   geom_errorbar(data=ad.treshhold.t, aes(y=Treshold, ymax=Treshold, ymin=Treshold), colour="#AA0000")+ theme(legend.position = "none")
...
topa_inta_data.median<-perobindall.cor[perobindall.cor$Image_Metadata_array<9,
                                       c("Cell_Intensity_MedianIntensity_Actin1amask_Nor","ImageNumber","FeatureIdx")]
...
ratiorankcorra<-ddply(topa_inta_data.median,"ImageNumber", summarise, 
                      TrMeanActin=mean(Cell_Intensity_MedianIntensity_Actin1amask_Nor, trimm=0.2),
                      FeatureIdx=unique(FeatureIdx))
...
#calculating statistics chi test
...
for (i in 1:length(unique(ratiorankcorrfff[,"FeatureIdx"]))){ 
  ...
  exp<-c(sum(neg_int_data.median$Cell_Intensity_MedianIntensity_Icam1amask_Nor >intersection.point.median)/nrow(neg_int_data.median),
         sum(neg_int_data.median$Cell_Intensity_MedianIntensity_Icam1amask_Nor<intersection.point.median)/nrow(neg_int_data.median))
...
ggplot(ratiorankcorrfff,aes(p.value.adj,p.value))+geom_point()+geom_hline(yintercept=0.05,colour="red")+
  geom_vline(xintercept=0.05,colour="blue")+ylim(0,0.05/2000)
# count total number of passed surfaces
nrow(ratiorankcorrfff.pv)

ggplot(rankration,aes(1:nrow(rankration),RatioTrMean))+geom_point()
...
dataicamav<-ddply(perobindall.cor[perobindall.cor$Image_Metadata_array<9,c("FeatureIdx", "ImageNumber",
                                                                           "Cell_Intensity_MedianIntensity_Actin1amask_Nor", 
                                                                           "Cell_Intensity_MedianIntensity_Icam1amask_Nor")],"ImageNumber", summarise,
                  FeatureIdx=unique(FeatureIdx),
                  Cell_Intensity_MedianIntensity_Icam1amask_Nor=mean(Cell_Intensity_MedianIntensity_Icam1amask_Nor,
                                                                          trimm=0.2),
                  Cell_Intensity_MedianIntensity_Actin1amask_Nor=mean(Cell_Intensity_MedianIntensity_Actin1amask_Nor,
                                                                           trimm=0.2))
...
forplottransf <- transform(forplotcoll[,c("FeatureIdx", "Cell_Intensity_MedianIntensity_Actin1amask_Nor",
                                          "Cell_Intensity_MedianIntensity_Icam1amask_Nor") ], FeatureIdx = factor(FeatureIdx, 
                                                                                                                       levels = unique(as.character(forplotcoll$FeatureIdx))))
library(reshape2)
forplottransfmelt<-melt(forplottransf, measure.vars =c("Cell_Intensity_MedianIntensity_Icam1amask_Nor",
                                                       "Cell_Intensity_MedianIntensity_Actin1amask_Nor"))
...
#######
##save results
save(bottomicam,topicam,file="Hitscalculated from frequences with adoptive treshhold_chi tets .RDATA")

#calculating statistics kruskal wallis test

for (i in 1:length(unique(ratiorankcorrfff[,"FeatureIdx"]))){ 
  temp <- ratiorankcorr[ratiorankcorr$FeatureIdx==ratiorankcorrfff[i,"FeatureIdx"],]
  pva<-wilcox.test(temp[,"Ratio"],ratiorankcorrn$Ratio )
  ratiorankcorrfff[i,"p.value_ut"]<-as.numeric(pva["p.value"])
  }
sorted.pvalue_ut<-ratiorankcorrfff[order(ratiorankcorrfff$p.value_ut),]

##select only surfaces that passess p value + correction
ratiorankcorrfff$p.value_ut.adj<-p.adjust(ratiorankcorrfff$p.value_ut, method= "BH")
ratiorankcorrfff.pv<-ratiorankcorrfff[ratiorankcorrfff$p.value_ut.adj<0.05,]
rankration<-ratiorankcorrfff.pv[order(ratiorankcorrfff.pv$RatioTrMean,
                                      ratiorankcorrfff.pv$p.value_ut.adj),c("FeatureIdx","RatioTrMean")]

ggplot(ratiorankcorrfff,aes(p.value_ut.adj,p.value_ut))+geom_point()+geom_hline(yintercept=0.05,colour="red")+
